import { mockAnswerCitations, mockArtifacts, mockEvidence, mockJobs, mockReviewItems, mockSteps } from './mock'
import type { AgentStep, Answer, Citation, ConversationMessage, Evidence, Job, ReviewItem } from '../types'
...
export async function getRun(jobId: string): Promise<{ job: Job; steps: AgentStep[] }> {
  if (USE_MOCK) return new Promise(resolve => setTimeout(() => resolve({ job: mockJobs.find(item => item.id === jobId) ?? mockJobs[0], steps: mockSteps }), 180))
...
export async function listArtifacts(jobId: string): Promise<Array<Record<string, unknown>>> {
  if (USE_MOCK) return jobId === 'job_8f21' ? mockArtifacts : []
  return request<Array<Record<string, unknown>>>(`/jobs/${jobId}/artifacts`)
}

export async function startJob(jobId: string): Promise<Record<string, unknown>> {
  if (USE_MOCK) {
    const job = mockJobs.find(item => item.id === jobId)
    if (job) { job.status = 'RUNNING'; job.progress = Math.max(job.progress, 5) }
    return { job_id: jobId, run_id: `run_${Date.now()}`, job_status: job?.status ?? 'RUNNING', run_status: 'INSPECTING' }
  }
  return request<Record<string, unknown>>(`/jobs/${jobId}/start`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() } })
}
...
export async function cancelJob(jobId: string): Promise<Job | null> {
  ...
  const raw = await request<Record<string, unknown>>(`/jobs/${jobId}/cancel`, { method: 'POST' })
  return { id: String(raw.job_id), title: `分析任务 · ${String(raw.job_id).slice(0, 8)}`, mode: String(raw.mode ?? 'video').toUpperCase() as Job['mode'], status: String(raw.status ?? 'CANCELLED').toUpperCase() as Job['status'], progress: Number(raw.progress ?? 0), createdAt: formatDate(raw.created_at), error: raw.error_code ? String(raw.error_code) : undefined, evidenceCount: 0 }
}

export async function rerunJob(jobId: string): Promise<Record<string, unknown>> {
  if (USE_MOCK) {
    const source = mockJobs.find(item => item.id === jobId)
    const job: Job = { ...(source ?? mockJobs[0]), id: `job_${Math.random().toString(16).slice(2, 8)}`, status: 'RUNNING', progress: 5, createdAt: '刚刚' }
    mockJobs.unshift(job)
    return { job_id: job.id, run_id: `run_${Date.now()}`, job_status: job.status, run_status: 'INSPECTING' }
  }
  return request<Record<string, unknown>>(`/jobs/${jobId}/rerun`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() } })
}

export async function retryJob(jobId: string): Promise<Record<string, unknown>> {
  if (USE_MOCK) {
    const job = mockJobs.find(item => item.id === jobId)
    if (job) { job.status = 'RUNNING'; job.progress = Math.max(job.progress, 5); job.error = undefined }
    return { job_id: jobId, run_id: `run_${Date.now()}`, job_status: job?.status ?? 'RUNNING', run_status: 'INSPECTING' }
  }
  return request<Record<string, unknown>>(`/jobs/${jobId}/retry`, { method: 'POST', headers: { 'Idempotency-Key': crypto.randomUUID() } })
}

export type LoginResponse = { access_token: string; token_type: string; user_id: string; workspace_id?: string | null }

export async function login(email: string, password: string): Promise<LoginResponse> {
  if (USE_MOCK) return { access_token: 'mock-token', token_type: 'bearer', user_id: 'mock-user', workspace_id: 'workspace_mock' }
  return request<LoginResponse>('/auth/login', { method: 'POST', body: JSON.stringify({ email, password }) })
}

export async function register(email: string, password: string, workspaceName: string): Promise<LoginResponse> {
  if (USE_MOCK) return { access_token: 'mock-token', token_type: 'bearer', user_id: 'mock-user', workspace_id: 'workspace_mock' }
  return request<LoginResponse>('/auth/register', { method: 'POST', body: JSON.stringify({ email, password, workspace_name: workspaceName }) })
}
...
export async function listConversations(jobId?: string): Promise<Conversation[]> {
  if (USE_MOCK) return [...mockConversations.values()].filter(item => !jobId || item.job_id === jobId)
...
export async function listReviewItems(filters: { jobId?: string; decision?: string; reviewerId?: string } = {}): Promise<ReviewItem[]> {
  if (USE_MOCK) return mockReviewItems.filter(item => !filters.jobId || item.job_id === filters.jobId)
...
export async function decideReview(reviewId: string, decision: 'APPROVED' | 'REJECTED' | 'MODIFIED' | 'MATERIALS_REQUESTED', note = '', revisedObservation?: Record<string, unknown>): Promise<Record<string, unknown>> {
  if (USE_MOCK) {
    const item = mockReviewItems.find(value => value.review_id === reviewId)
    if (item) { item.status = 'DECIDED'; item.decision = decision; item.reason = note; item.revised_observation = revisedObservation ?? null; item.revision += 1 }
    return { review_id: reviewId, status: 'DECIDED', decision }
  }
