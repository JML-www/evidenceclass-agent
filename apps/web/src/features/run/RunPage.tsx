...
export function RunPage() {
  ...
  const runQuery = useQuery({
    ...
    refetchInterval: query =>
      ...
      query.state.data?.job.status === 'QUEUED'
        ? 5_000
        : false,
  ...
  React.useEffect(() => {
    ...
    streamJobEvents(jobId, event => {
      const progress = Number(
        event.payload.progress ?? event.payload.job_progress ?? NaN,
      )
      const status = String(
        event.payload.status ?? event.payload.job_status ?? '',
      ).toUpperCase()
        ...
        action={
          <div className="header-actions">
            <button
              ...
              disabled={
                !data ||
                ['SUCCEEDED', 'FAILED', 'CANCELLED'].includes(data.job.status)
              }
        ...
        <div className="run-grid">
          ...
          <aside className="run-side">
            <section className="panel">
              ...
              <KeyValue
                label="产物"
                value={`${data.steps.length} 个执行节点`}
              />
            </section>
            <section className="panel callout">
              <span>ⓘ</span>
              <p>
                时间线只记录结构化计划、工具输入摘要和结果摘要，不暴露模型私有思维链。
              </p>
            </section>
          </aside>
        </div>
